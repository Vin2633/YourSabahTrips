import { MapPin, Clock, Users, Filter, TrendingUp } from 'lucide-react';
...
export function PackagesPage({ darkMode, initialFilters }: PackagesPageProps) {
  const [selectedLocation, setSelectedLocation] = useState<string>(initialFilters?.location || '');
  const [selectedActivity, setSelectedActivity] = useState<string>(initialFilters?.activity || '');
  ...
  return (
    <div className={`min-h-screen py-12 px-4 ${darkMode ? 'bg-[oklch(0.3_0.05_160)]' : 'bg-[#f0f9f4]'}`}>
      <div className="max-w-7xl mx-auto">
        ...
        {/* Sort By */}
        <div className={`rounded-xl p-6 mb-8 shadow-lg border-2 ${
          darkMode ? 'bg-[oklch(0.35_0.05_160)] border-[oklch(0.4_0.05_160)]' : 'bg-white border-[#95d5b2]'
        }`}>
          <div className="flex items-center gap-3 mb-4">
            <TrendingUp className="w-5 h-5 text-[#52b788]" />
            <h3 className={darkMode ? 'text-[oklch(0.9_0.03_160)]' : 'text-[#1b4332]'}>Sort By</h3>
          </div>
          <select
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value as any)}
            className={`w-full md:w-64 px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#52b788] ${
              darkMode 
                ? 'bg-[oklch(0.4_0.05_160)] border-[oklch(0.45_0.05_160)] text-[oklch(0.9_0.03_160)]' 
                : 'bg-[#f0f9f4] border-[#95d5b2] text-[#1b4332]'
            }`}
